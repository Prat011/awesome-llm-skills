...
- [cowork-to-code-bridge](https://github.com/abhinaykrupa/cowork-to-code-bridge) - Connects a Claude Cowork session (or any sandboxed agent) to Claude Code running on your own Mac or Linux box, so it can build, run tests, git push, and inspect the real machine through an async file-based bridge with per-task budget caps and permission scopes. *By [@abhinaykrupa](https://github.com/abhinaykrupa)*
