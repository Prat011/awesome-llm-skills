...
- [TWZRD Agent Intel](https://intel.twzrd.xyz) - Trust infrastructure MCP server for AI agents on Solana. Provides on-chain agent scoring, pre-dispatch trust gating, and signed V5 receipts. Free tools: `resolve_agent`, `score_agent`, `preflight_check`, `verify_trust_receipt`. Paid: `get_trust_receipt` (x402/USDC). Zero-install Streamable HTTP endpoint.
