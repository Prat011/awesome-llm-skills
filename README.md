...
- [Remio](./remio/) - Local-first personal knowledge base skill for searching notes, files, webpages, recordings, emails, messages, and indexed context through the Remio CLI. Requires the Remio desktop client; the skill is an agent interface to the client, not a standalone memory backend.
