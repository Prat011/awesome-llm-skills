...
- [nika-authoring](https://github.com/supernovae-st/nika-agents) - Author-and-check loop for Nika AI workflows: the agent writes a `.nika.yaml` DAG, `nika check` audits schema, permits and an honest cost floor before a single token is spent, runs leave tamper-evident traces. *By [@ThibautMelen](https://github.com/ThibautMelen)*
