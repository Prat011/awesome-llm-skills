...
- [Bernstein Orchestrator](./bernstein-orchestrator/) - Use Bernstein to orchestrate parallel CLI coding agents (Claude Code, Codex CLI, Gemini CLI, OpenHands, Cursor, Aider, and 40+ more) on a single repository, with role-scoped agents in isolated git worktrees, quality gates, cost tracking, and a first-class MCP server.
