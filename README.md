...
- [QAIA](https://github.com/QAIA-Project/QAIA) - Takes a user story to a traceable Gherkin test book with stable scenario IDs and a requirement coverage matrix, then to native Playwright tests. Ambiguous acceptance criteria are surfaced as named open questions rather than silently resolved. Markdown skills only, no API key, no MCP server. *By [@Opaland](https://github.com/Opaland)*
